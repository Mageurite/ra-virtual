...
import TutorsPage from './components/admin/TutorsPage';
import TutorStudentsPage from './components/admin/TutorStudentsPage';
...
function StudentSessionRoute({ isLoggedIn, userRole, onLogout }) {
  const { tutorId } = useParams();
  if (!isLoggedIn || userRole !== 'student') {
    return <Navigate to={`/t/${tutorId}/login`} replace />;
  }
  return <HomePage onLogout={onLogout} />; // 先用你现有 HomePage 当学生端页面
}

function StudentLoginRoute({ isLoggedIn, userRole, onLoginSuccess }) {
  const { tutorId } = useParams();
  if (isLoggedIn && userRole === 'student') {
    return <Navigate to={`/t/${tutorId}`} replace />;
  }
  return <StudentLoginPage onLoginSuccess={onLoginSuccess} />;
}

// 兼容旧 /session 路由 -> /t 路由
function LegacyStudentLoginRedirect() {
  const { tutorId } = useParams();
  return <Navigate to={`/t/${tutorId}/login`} replace />;
}
function LegacyStudentSessionRedirect() {
  const { tutorId } = useParams();
  return <Navigate to={`/t/${tutorId}`} replace />;
}
      ...
      <BrowserRouter>
        <Routes>
          ...
          >
            ...
            <Route path="tutors/:tutorId" element={<TutorStudentsPage />} />
            <Route path="logs" element={<AuditLogsPage />} />
          </Route>

          {/* Student 新路由：Tutor URL */}
          <Route
            path="/session/:tutorId/login"
            element={
              <StudentLoginRoute
