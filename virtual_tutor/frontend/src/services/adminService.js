...
const API_ENDPOINTS = {
    ...
    SESSION_LIST: '/admin/sessions',
    SESSION_DETAIL: (sessionId) => `/admin/sessions/${sessionId}`,
    SESSION_MESSAGES: (sessionId) => `/admin/sessions/${sessionId}/messages`,
    // Avatar endpoints
...
class AdminService {
    ...
    async getSessionList(params = {}) {
        try {
            const { page = 1, per_page = 20, tutor_id, student_id } = params;
            const apiParams = { page, per_page };
            if (tutor_id) apiParams.tutor_id = tutor_id;
            if (student_id) apiParams.student_id = student_id;

            const response = await http.get(API_ENDPOINTS.SESSION_LIST, { params: apiParams });
            return { success: true, data: response, message: 'Successfully obtained session list' };
        } catch (error) {
            return { success: false, error: errorHandler.handleApiError(error), message: errorHandler.showError(error) };
        }
    }
    ...
    async getSessionDetail(sessionId) {
        try {
            const response = await http.get(API_ENDPOINTS.SESSION_DETAIL(sessionId));
            return { success: true, data: response, message: 'Successfully obtained session detail' };
        } catch (error) {
            return { success: false, error: errorHandler.handleApiError(error), message: errorHandler.showError(error) };
        }
    }
    ...
    async getSessionMessages(sessionId, params = {}) {
        try {
            ...
            const response = await http.get(API_ENDPOINTS.SESSION_MESSAGES(sessionId), {
            params: { page, per_page }
            });
            return { success: true, data: response, message: 'Successfully obtained session messages' };
        } catch (error) {
            return { success: false, error: errorHandler.handleApiError(error), message: errorHandler.showError(error) };
        }
    }
